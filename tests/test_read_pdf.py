...
def test_cleanup_path_only_removes_own_temp_dirs(tmp_path):
    from zotero_mcp.tools import read_pdf

    unrelated_dir = tmp_path / "not_zotero_pdf"
    unrelated_dir.mkdir()
    unrelated_file = unrelated_dir / "paper.pdf"
    unrelated_file.write_bytes(b"%PDF")

    read_pdf._cleanup_path(str(unrelated_file))

    assert unrelated_dir.exists()
    assert unrelated_file.exists()

    managed_dir = tmp_path / "zotero_pdf_abc123"
    managed_dir.mkdir()
    managed_file = managed_dir / "paper.pdf"
    managed_file.write_bytes(b"%PDF")

    monkeypatch_temp = read_pdf.tempfile.gettempdir
    try:
        read_pdf.tempfile.gettempdir = lambda: str(tmp_path)
        read_pdf._cleanup_path(str(managed_file))
    finally:
        read_pdf.tempfile.gettempdir = monkeypatch_temp
